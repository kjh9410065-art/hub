/* HUB 1.6: 추천 화면의 목적 아이콘과 탐색 메뉴를 최신 공통 디자인으로 통일합니다. */
"use client";
...
// 신규 파일명으로 연결해 이전 배포본의 아이콘 캐시와도 확실하게 분리합니다.
const goals = [
...
export default function RecommendPage() {
  ...
  return <main className="recommendPage"><div className="recommendShell">
    ...
    <section className="recommendIntro"><div className="eyebrow">HUB 1.6 · PERSONAL RECOMMEND</div><h1>조건까지 반영해서<br /><span>{selectedGoal?.[2]}</span>을 찾아보세요.</h1><p>{selectedGoal?.[3]}</p></section>
    ...
    <section className="conditionCard featureCondition"><h2>04. 가장 중요한 기능</h2><div className="choiceGrid">{featureOptions.map(([id, label]) => <button className={`choice ${feature === id ? "active" : ""}`} key={id} onClick={() => setFeature(id)}>{label}</button>)}</div></section>
