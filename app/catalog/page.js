    ...
    <section className="catalogBody">
      ...
      {list.length > 0 && <div className="catalogGrid">{list.map((s, index) => {
        ...
        return <article className={`catalogCard ${affiliate ? "affiliateCard" : ""} ${index < 3 ? "catalogTopResult" : ""}`} key={s.id}>
          ...
          <div className="catalogActions"><Link href={`/services/${s.id}`}>자세히 보기</Link><a href={getOutboundUrl(s)} target="_blank" rel="nofollow sponsored noopener noreferrer" onClick={() => openService(s, "catalog")}>공식 사이트</a><button type="button" onClick={() => toggleCompare(s.id)}>{compare.includes(s.id) ? "비교 선택됨" : "비교하기"}</button><button type="button" aria-label={isFavorite ? `${s.name} 즐겨찾기 해제` : `${s.name} 즐겨찾기`} title={isFavorite ? "즐겨찾기 해제" : "즐겨찾기"} className={`catalogFavorite ${isFavorite ? "active" : ""}`} onClick={() => toggleFavorite(s.id)}>♥</button></div>
