...
export default function RecommendPage() {
  ...
  // 자연어 검색어에서 명확하게 드러나는 목적·예산·난이도·기능을 읽어냅니다.
  ...
  const results = useMemo(() => {
    const ranked = rankServices(catalog, {
      goal: effectiveGoal,
  ...
  return <main className="recommendPage"><div className="recommendShell">
    ...
    <section className="recommendIntro">
      <div className="eyebrow">HUB 2.0 · PERSONAL RECOMMEND</div>
      <h1>조건까지 반영해서<br/><span>{selectedGoal[2]}</span>을 찾아보세요.</h1>
      <p>{selectedGoal[3]}</p>
    </section>

    <section className="recommendSearch recommendIntentSearch">
      <label htmlFor="recommend-query">원하는 조건을 한 문장으로 입력해도 됩니다.</label>
      <div className="recommendSearchRow">
        <input id="recommend-query" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="예: 무료로 쇼츠 만들고 싶어" autoComplete="off" />
        {query && <button type="button" onClick={() => setQuery("")}>지우기</button>}
      </div>
      {intent.labels.length > 0 && <div className="intentLabels" aria-label="검색어에서 인식한 조건">{intent.labels.map((label) => <span key={label}>{label}</span>)}</div>}
    </section>

    <section className="conditionGrid">
      <div className="conditionCard"><h2>01. 만들고 싶은 것</h2><div className="choiceGrid">{goals.map(([id, icon, label]) => <button type="button" className={`choice ${effectiveGoal === id ? "active" : ""}`} key={id} onClick={() => { setGoal(id); setQuery(""); }}><img src={icon} alt=""/><span>{label}</span></button>)}</div></div>
      <div className="conditionCard"><h2>02. 예산</h2><div className="choiceGrid">{budgetOptions.map(([id, label]) => <button type="button" className={`choice ${effectiveBudget === id ? "active" : ""}`} key={id} onClick={() => { setBudget(id); setQuery(""); }}>{label}</button>)}</div></div>
      <div className="conditionCard"><h2>03. 개발 경험</h2><div className="choiceGrid">{skillOptions.map(([id, label]) => <button type="button" className={`choice ${effectiveSkill === id ? "active" : ""}`} key={id} onClick={() => { setSkill(id); setQuery(""); }}>{label}</button>)}</div></div>
    </section>
