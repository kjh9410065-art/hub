...
// 모든 화면이 이 모듈을 사용하므로 검색 경험이 일관되게 유지됩니다.
...
const purposeAliases = {
  쇼츠:"shorts",숏츠:"shorts",릴스:"shorts",틱톡:"shorts",숏폼:"shorts",이미지:"image",사진:"image",상품사진:"image",상품이미지:"image",썸네일:"image",
  영상:"video",비디오:"video",음성:"voice",목소리:"voice",더빙:"voice",tts:"voice",stt:"voice",챗봇:"chat",채팅봇:"chat",llm:"chat",모델:"chat",
...
export function scoreSearch(service,query){
  ...
  score+=matchedOriginalTerms.length*3;if(terms.length>1&&matchedOriginalTerms.length===terms.length)score+=12;
...
// 검색 결과가 없으면 검색 의도와 조건을 보존하면서 대체 서비스를 찾습니다.
export function getSearchSuggestions(services,query,options={}){
  const{raw,purposes,conditions}=buildTerms(query);if(!raw)return[];
  const onlyFree=Boolean(options.onlyFree),onlyApi=Boolean(options.onlyApi);
  return services.map((service,index)=>{
    if(onlyFree&&!service.free)return null;if(onlyApi&&!service.api)return null;
    let score=0;
  ...
  }).filter(Boolean).filter((item)=>item.score>0).sort((a,b)=>b.score-a.score||a.index-b.index).slice(0,3).map((item)=>item.service);
}
