/* HUB 0.8 메인 화면: 목적 → 검색 → 필터 → 추천 → 상세 정보 → 무료 도구까지 연결합니다. */
"use client";
...
export default function Home() {
  ...
  useEffect(() => {
    try { setFavorites(JSON.parse(localStorage.getItem("hub-favorites") || "[]")); } catch { setFavorites([]); }
  }, []);
  ...
  const selectTask = (id) => { setSelectedTask(id); setQuery(""); setCategory("전체"); setSortMode("recommend"); setTimeout(() => document.getElementById("recommend")?.scrollIntoView({ behavior:"smooth", block:"start" }),0); };
  const search = (value) => { setQuery(value); setCategory("전체"); setTimeout(() => document.getElementById("recommend")?.scrollIntoView({ behavior:"smooth", block:"start" }),0); };

  return (
    <main className="page">
      <header className="header">
        <a className="logo" href="#top">HUB</a>
        <nav><a href="#tasks">목적별 찾기</a><a href="#recommend">추천 결과</a><a href="/tools">무료 도구</a><a href="/compare">비교하기</a></nav>
      </header>

      <section id="top" className="hero">
        <div className="badge">HUB 0.8 · 목적 기반 추천</div>
        <h1>뭘 써야 할지 모르겠다면,<br /><span>목적을 알려주세요.</span></h1>
        <p>서비스 이름이 없어도 괜찮아요. 만들고 싶은 것을 선택하면 됩니다.</p>
        <div className="searchBox"><input value={query} onChange={(e)=>setQuery(e.target.value)} onKeyDown={(e)=>e.key === "Enter" && search(query)} placeholder="예: 이미지, 음성, 검색, GPU..." aria-label="서비스 검색" /><button onClick={()=>search(query)}>찾아보기</button></div>
      ...
      <section id="tasks" className="section">
        ...
        <div className="taskGrid">{tasks.map((task)=><button key={task.id} className={`taskCard ${selectedTask===task.id&&!query?"selected":""}`} onClick={()=>selectTask(task.id)}><span className="taskIcon">{task.icon}</span><span className="taskText"><b>{task.title}</b><small>{task.desc}</small></span><span className="arrow">→</span></button>)}</div>
      </section>

      <section id="recommend" className="recommendSection">
        <div className="sectionHead"><div><div className="eyebrow">STEP 02 · RECOMMEND</div><h2>{query?`“${query}” 검색 결과`:`${selectedTaskInfo?.title}에 맞는 서비스`}</h2></div><div className="sortButtons"><button className={sortMode==="recommend"?"active":""} onClick={()=>setSortMode("recommend")}>추천순</button><button className={sortMode==="favorite"?"active":""} onClick={()=>setSortMode("favorite")}>즐겨찾기</button><button className={sortMode==="name"?"active":""} onClick={()=>setSortMode("name")}>이름순</button></div></div>
        <div className="filterRow"><span>분류</span><div className="categoryFilters">{categories.map((item)=><button key={item} className={category===item?"active":""} onClick={()=>setCategory(item)}>{item}</button>)}</div></div>
        ...
        <div className="serviceGrid">{results.map((service,index)=>{const score=recommendationScores[selectedTask]?.[service.id]||60;return <article className={`serviceCard ${index===0&&!query?"featured":""}`} key={service.id}><div className="serviceTop"><div className="serviceIcon">{service.icon}</div><button className={`favorite ${favorites.includes(service.id)?"saved":""}`} onClick={()=>toggleFavorite(service.id)} aria-label={`${service.name} 즐겨찾기`}>{favorites.includes(service.id)?"★":"☆"}</button></div><div className="cardMeta"><span className="category">{service.category}</span><span className="match">{score}% MATCH</span></div><h3>{service.name}</h3><p>{service.desc}</p><div className="bestFor"><span>추천 용도</span><b>{service.bestFor}</b></div><div className="tags">{service.tags.map(tag=><span key={tag}>{tag}</span>)}</div><div className="serviceActions"><button onClick={()=>setSelectedService(service)}>자세히 보기</button><a href={service.url} target="_blank" rel="noreferrer">공식 사이트 ↗</a></div></article>})}{results.length===0&&<div className="empty"><b>찾는 서비스가 아직 없어요.</b><span>다른 키워드로 검색하거나 분류를 바꿔보세요.</span><button onClick={()=>{setQuery("");setCategory("전체");}}>추천 다시 보기</button></div>}</div>
      </section>

      <section id="compare" className="allSection"><div className="eyebrow">ALL SERVICES · {services.length}</div><h2>서비스를 한눈에 둘러보세요.</h2><p className="sectionSub">관심 있는 서비스를 누르면 상세 정보를 확인할 수 있습니다.</p><div className="miniGrid">{services.map(service=><button key={service.id} className="miniCard" onClick={()=>setSelectedService(service)}><span>{service.icon}</span><div><b>{service.name}</b><small>{service.category}</small></div><strong>{favorites.includes(service.id)?"★":"→"}</strong></button>)}</div></section>

      <section className="toolPromo"><div><div className="eyebrow">FREE TOOLS</div><h2>서비스를 고르기 전에<br /><span>비용부터 확인해보세요.</span></h2><p>AI API 비용 계산기와 비교·가이드를 무료로 이용할 수 있습니다.</p></div><div className="toolPromoActions"><a href="/tools/cost">비용 계산기 →</a><a href="/tools">무료 도구 전체 보기 →</a></div></section>

      <footer><b>HUB</b><span>목적에 맞는 AI·개발 서비스를 더 쉽게 찾을 수 있도록 계속 확장합니다.</span></footer>
      {selectedService&&<div className="modalBackdrop" onClick={()=>setSelectedService(null)}><div className="modal" onClick={(e)=>e.stopPropagation()}><button className="modalClose" onClick={()=>setSelectedService(null)} aria-label="닫기">×</button><div className="modalIcon">{selectedService.icon}</div><div className="category">{selectedService.category}</div><h3>{selectedService.name}</h3><p>{selectedService.desc}</p><div className="modalInfo"><span>추천 용도</span><b>{selectedService.bestFor}</b></div><div className="modalInfo"><span>이 서비스가 좋은 이유</span><div>{selectedService.strengths.map(item=><b className="reason" key={item}>✓ {item}</b>)}</div></div><div className="tags">{selectedService.tags.map(tag=><span key={tag}>{tag}</span>)}</div><div className="modalActions"><button onClick={()=>toggleFavorite(selectedService.id)}>{favorites.includes(selectedService.id)?"★ 저장됨":"☆ 저장하기"}</button><a className="modalLink" href={selectedService.url} target="_blank" rel="noreferrer">공식 사이트 방문 ↗</a></div></div></div>}
    </main>
  );
}
