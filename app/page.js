...
export default function HomePage() {
  ...
  useEffect(() => {
    localStorage.setItem("hub-compare", JSON.stringify(compare));
  }, [compare]);

  const ranked = useMemo(() => {
    return rankServices(catalog, {
      goal: selectedTask,
      budget: "any",
      skill: "any",
  ...
  const results = useMemo(() => {
    ...
    return ordered.filter((service) => {
      ...
      return searchable(service).includes(needle);
    });
    ...
    <main className="homePage">
      ...
      <section className="heroSection">
        <div className="heroCopy">
          ...
          <div className="heroActions">
            <Link href="/recommend" className="primaryButton">맞춤 추천 받기</Link>
            <Link href="/catalog" className="secondaryButton">전체 카탈로그 보기</Link>
          </div>
        </div>
        <div className="heroStat">
          <strong>{catalog.length}</strong>
          <span>서비스를 한곳에서 비교</span>
        </div>
      </section>

      <section className="taskSection">
        <div className="sectionHeading">
          <div><p className="sectionKicker">START HERE</p><h2>만들고 싶은 것을 선택하세요</h2></div>
      ...
      <section className="searchSection">
        ...
        <div className="filterRow">
          ...
          <div className="filterGroup featureGroup" aria-label="기능 필터">
      ...
      <section className="resultSection">
        <div className="resultHeading">
          <div><p className="sectionKicker">RECOMMENDED</p><h2>{activeTask.title}에 맞는 서비스</h2></div>
          <span>{results.length}개 결과</span>
        </div>
        {results.length === 0 ? (
          <div className="emptyState"><strong>조건에 맞는 서비스가 없습니다.</strong><p>필터를 조금 완화하거나 전체 카탈로그에서 다시 찾아보세요.</p><Link href="/catalog">카탈로그 열기</Link></div>
        ) : (
          <div className="serviceGrid">
            {results.map((service, index) => {
              ...
              return (
                <article className={`serviceCard ${index === 0 && !query ? "topMatch" : ""}`} key={service.id}>
                  <div className="cardTop">
                    <div className="serviceIconWrap"><img src={service.icon} alt="" className="serviceIconImage" /></div>
                    <div className="cardBadges">
                      {index === 0 && !query && <span className="matchBadge">TOP MATCH</span>}
                      {service.free && <span>무료 사용 가능</span>}
                      {service.api && <span>API</span>}
                    </div>
                    <button type="button" className={`favoriteButton ${isFavorite ? "isFavorite" : ""}`} onClick={() => toggleFavorite(service.id)} aria-label={isFavorite ? "즐겨찾기 해제" : "즐겨찾기 추가"}>{isFavorite ? "즐겨찾기됨" : "즐겨찾기"}</button>
                  </div>
                  <Link href={`/services/${service.id}`} className="serviceName">{service.name}</Link>
                  <p className="serviceCategory">{service.category} · {service.difficulty || "보통"}</p>
                  ...
                  <div className="cardBottom">
                    <Link href={`/services/${service.id}`} className="detailLink">자세히 보기</Link>
                    <button type="button" className={`compareButton ${isCompared ? "selected" : ""}`} onClick={() => toggleCompare(service.id)}>{isCompared ? "비교함에서 제거" : "비교하기"}</button>
                  </div>
                </article>
              );
            })}
      ...
      {compare.length > 0 && (
        <div className="compareBar">
          <div><strong>비교함</strong><span>{compare.length}/4개 선택</span></div>
          <div className="compareBarActions"><button type="button" onClick={() => setCompare([])}>전체 해제</button><Link href="/compare">비교 화면 열기</Link></div>
        </div>
      )}

      <footer className="homeFooter">
        <div><strong>HUB</strong><span>목적에 맞는 AI·개발 서비스 탐색</span></div>
        <div className="footerLinks"><Link href="/recommend">추천</Link><Link href="/catalog">카탈로그</Link><Link href="/compare">비교</Link></div>
      </footer>
    </main>
  );
}
